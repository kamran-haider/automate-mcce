...
for topDir in dirs_in_pdb_files:
    ...
    for pdb_file in onlyfiles:
        if "_fixed_ph7.4.pdb" in pdb_file:
            ...
            sys_call = 'cp /home/salah/johnChodera_project_on_github/mcce-charges/mcce/run.prm ' + \
                destination_runs + pdbfile[0] + \
                '/' + pdb_file[:4] + '_together'
            os.system(sys_call)
            ...
            sys_call = 'mv ' + destination_runs + pdbfile[0] + '/' + pdb_file[
                :4] + '_together' + '/run.prm2 ' + destination_runs + pdbfile[0] + '/' + pdb_file[:4] + '_together' + '/run.prm'
            os.system(sys_call)
            mysubmit = destination_runs + \
                pdbfile[0] + '/' + pdb_file[:4] + '_together'
            # print mysubmit
            os.chdir(mysubmit)
            qsub_call = "qsub %s"
